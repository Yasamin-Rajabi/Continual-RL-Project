"""All plotting and table/CSV writing for the continual Meta-World benchmark.
...
TRAIN_METRICS = {
    ...
    "charts/test_task_error": ("Evaluation object-to-target distance", "eval_task_error"),
...
SEQUENCE_METRICS = {
    ...
    "analysis/buffer/mean_task_error": ("Merge-buffer mean object-to-target distance", "buffer_task_error"),
}
...
def plot_retention(args, suite, conditions, all_payloads):
    ...
    for condition in conditions:
        ...
        for metric in ("return", "task_error", "success"):
    ...
    for condition in conditions:
        ...
        final_errors = np.asarray([
            np.asarray(p["task_error"], dtype=np.float64)[-1].mean()
            for p in all_payloads[condition]
        ])
        ax.errorbar(
    ...
    ax.legend(loc="best")
    fig.tight_layout()
    fig.savefig(out_dir / "tradeoff_return_vs_task_error.png", dpi=180)
    plt.close(fig)
    ...
    for condition in conditions:
        for payload in all_payloads.get(condition, []):
            ...
            rows.append({
                ...
                "final_avg_task_error_all_eval_tasks": float(err[-1].mean()),
                "final_avg_success_all_eval_tasks": float(succ[-1].mean()),
                "final_avg_return_seen": float(avg_ret[-1]),
                "final_avg_task_error_seen": float(avg_err[-1]),
