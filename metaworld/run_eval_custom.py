...
def parse_args():
    ...
    # Key settings for the metrics and the warmup / test-time-adaptation phase
    p.add_argument("--retention-eval-episodes", type=int, default=3)
    p.add_argument("--test-adapt-steps", type=int, default=1000,
                   help="Number of steps to adapt alpha logits during test-time evaluation (e.g. 1000 instead of 5000)")
    
    p.add_argument("--save-root", default="agents_metaworld")
    ...
    # Main argument: a custom dict of directories / hyperparameters
    p.add_argument(
...
def main():
    ...
    for suite in args.task_suites:
        ...
        # NOTE: the training path (train_chain) is removed entirely so it cannot fire.

        # 2. Draw plots and diagnostics using the custom paths
        plots.plot_sequence_diagnostics(args, suite, conditions)
        plots.plot_zero_shot(args, suite, conditions)

        # 3. Compute the retention matrix, applying the new test-time adaptation steps
        ...
        # 4. Compute the survey metrics (A_N, FG, BWT, FT)
