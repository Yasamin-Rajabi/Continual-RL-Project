...
def source_fingerprint(root=None) -> str:
    ...
    for name in SOURCE_CANDIDATES:
        ...
        h.update(b"\0")
    # When Ant calibration is supplied from an external path, its contents are
    # still part of the benchmark definition and therefore part of checkpoint
    # identity. The normal local ant_calibration.json path is already covered
    # by SOURCE_CANDIDATES above.
    external_calibration = os.environ.get("CKA_ANT_CALIBRATION_FILE")
    if external_calibration:
        calibration_path = pathlib.Path(external_calibration).expanduser()
        if calibration_path.exists() and calibration_path.is_file():
            found += 1
            h.update(b"CKA_ANT_CALIBRATION_FILE\0")
            h.update(calibration_path.read_bytes())
            h.update(b"\0")
