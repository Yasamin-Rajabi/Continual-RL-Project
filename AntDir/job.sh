...
EXPERIMENT_ROOT="${EXPERIMENT_ROOT:-$BASE_STORAGE/ethos_student_antdir_300k}"
...
if [[ "$MODE" != "--worker" && "$MODE" != "--aggregate-worker" ]]; then
    ...
    if [[ "$AGGREGATE" -eq 1 ]]; then
        echo "============================================================"
        echo "Submitting AntDir aggregation/evaluation jobs"
    ...
    echo "============================================================"
    echo "Submitting AntDir main runs with one SLURM job per seed"
    echo "Methods: ${VARIANTS[*]}"
    echo "Modes: ${EVAL_MODES[*]}"
    echo "Seeds: ${MAIN_SEEDS[*]}"
    echo "Comment: ${COMMENT:-<none>}"
...
if [[ "$MODE" == "--worker" ]]; then
    ...
    echo "[done aggregate] survey metrics: $RUN_ROOT/plots/ant_dir/survey_metrics.csv"
