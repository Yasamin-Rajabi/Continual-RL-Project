"""Numeric metrics for the continual MiniGrid benchmark.

The paper pipeline uses ``charts/test_success`` as the normalized continual-RL
performance signal p_i(t): A_N, FG, and BWT are recomputed from saved
checkpoints with the same frozen-policy evaluation protocol used by the other
environments. Forward transfer is measured only on first encounters of new
tasks. ``FT_success`` compares success-curve AUC to matched scratch runs;
``FT_return`` applies the same headroom normalization to MiniGrid episodic
return, whose upper bound is 1.0. Learning-curve metrics are read from
``scalars.csv`` or TensorBoard logs, while checkpoint metrics use the finalized
policy pool.

This file performs computation/caching only; plotting lives in ``plots.py``.
"""
...
ERROR_KEY = "task_error"
EPISODIC_SUCCESS = True
# MiniGrid success reward is bounded by one; this makes the return-based FT
# normalization well-defined and directly comparable across the DoorKey suite.
...
def build_retention_matrix(args, suite, condition, seed, device):
    ...
    for seq_idx, trained_task in enumerate(args.task_sequence):
        ...
        rows = {metric: [] for metric in ("return", "success", ERROR_KEY, "evaluation_interactions", "adaptation_interactions")}
...
def compute_forward_transfer_return(args, suite, condition, seed, scratch_seeds, scratch_total_timesteps):
    ...
    MiniGrid episodic return is already bounded in [0, 1], so the environment-
    specific upper bound is exactly one.
    """
