...
def _mean_normalised(values, steps):
    """Mean of values over the logged step range, clipped to [0,1].

    MiniGrid episodic return is already bounded by 1, so this is just the time
    average of the evaluation curve and can be fed straight into the survey's
    forward-transfer formula alongside the success-rate version.
    """
    span = float(steps[-1] - steps[0])
    if span <= 0.0:
        return None
    return float(_trapz(np.clip(values, 0.0, 1.0), steps)) / span
...
def compute_forward_transfer_return(args, suite, condition, seed, scratch_seeds, scratch_total_timesteps):
    ...
    The HalfCheetah version used the shortcut FT_i = 1 - R_i/R_i^b, valid only
    when r_max = 0. MiniGrid rewards are non-negative and an episode returns
    1 - 0.9*(steps/max_steps) on success and 0 otherwise, so episodic return is
    ALREADY in [0,1] and no per-task reference scaling is needed. We therefore
    apply the same literal formula as compute_forward_transfer_success.

    Report FT_success as the primary number: success is the benchmark's own
    criterion and return only adds a step-count-efficiency term on top of it.
    """
    per_position = []
    per_position_index = []
    scratch_variant = _scratch_variant(args, condition)
    for seq_idx, task_id in _first_unseen_positions(args.task_sequence):
        ...
        if run_steps.size < 2:
            continue
        # Mean normalized performance: the trapezoid integral divided by the
        # curve's own step span. MiniGrid return is already in [0,1], so no
        # further scaling is applied. Each curve is normalized by ITS OWN span
        # so a continual run stays comparable to a baseline that logged a
        # slightly different step range.
        auc = _mean_normalised(run_values, run_steps)
        if auc is None:
            continue
        ...
        for b_seed in scratch_seeds:
            ...
            if b_steps.size >= 2:
                b_auc = _mean_normalised(b_values, b_steps)
                if b_auc is not None:
                    baseline_aucs.append(b_auc)
        if not baseline_aucs:
            continue
