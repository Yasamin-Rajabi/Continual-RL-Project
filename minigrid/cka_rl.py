...
from policy_utils import (
    bound_log_std, diagonal_gaussian_kl, symmetric_diagonal_gaussian_kl,
    categorical_kl, symmetric_categorical_kl,
)
...
from policy_composition import categorical_summary, stacked_head_forward


class CkaRlAgent(PolicySpaceMixin, nn.Module):
    """Shared encoder + an aligned pool of two-head policy components.
    ...
    def forward(self, x):
        if self.composition_space == "policy":
            ...
            return categorical_summary(*self.policy_components(x))
        features = self.fc(x)
        z = (
    ...
    def _symmetric_divergence(self, head_a, head_b, other_a, other_b):
        """Symmetric policy divergence, one scalar per state."""
        if self.discrete:
            return symmetric_categorical_kl(head_a + head_b, other_a + other_b)
        return symmetric_diagonal_gaussian_kl(head_a, head_b, other_a, other_b)

    def _forward_divergence(self, teacher_a, teacher_b, student_a, student_b):
        """Forward KL used for behavior-preserving distillation."""
        if self.discrete:
            return categorical_kl(teacher_a + teacher_b, student_a + student_b)
        return diagonal_gaussian_kl(teacher_a, teacher_b, student_a, student_b)
    ...
    def _distill_pair(self, idx1: int, idx2: int):
        """KL-distill two aligned policy entries into one student.
        ...
        with torch.no_grad():
            ...
            teacher_logstd = (
                teacher_raw_logstd if self.discrete else bound_log_std(teacher_raw_logstd)
            )
        ...
        for epoch in range(1, self.distill_epochs + 1):
            ...
            for start in range(0, shuffled.numel(), self.distill_batch_size):
                ...
                student_logstd = (
                    student_raw_logstd if self.discrete else bound_log_std(student_raw_logstd)
                )
        ...
        with torch.no_grad():
            def metrics(indices):
                ...
                sm, sl_raw = student_outputs(z[indices])
                sl = sl_raw if self.discrete else bound_log_std(sl_raw)
                kl_values = self._forward_divergence(
                    teacher_mean[indices], teacher_logstd[indices], sm, sl
                )
...
class FrozenCkaPolicy(nn.Module):
    ...
    def __init__(self, snapshot):
        ...
        self.discrete = bool(snapshot.get("discrete", True))
    ...
    def policy_components(self, obs):
        ...
        if self.composition_space == "policy":
            ...
            return (stacked_head_forward(z, heads[0]),
                    stacked_head_forward(z, heads[1]), self.mixture_weights)
        head_a, head_b = self._head(z, "mean"), self._head(z, "logstd")
        return head_a[:, None, :], head_b[:, None, :], head_a.new_ones(1)

    def forward(self, obs):
        if self.composition_space == "policy":
            return categorical_summary(*self.policy_components(obs))
        features = self.fc(obs)
        z = (
