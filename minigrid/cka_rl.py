...
class CkaRlAgent(nn.Module):
    ...
    def __init__(
        ...
        # Discrete action spaces (MiniGrid) keep the same two-head pool layout,
        # but the two heads jointly parameterize ONE categorical distribution:
        # logits = head_a(z) + head_b(z). This is an over-parameterization,
        # equivalent in expressiveness to a single wider logits head, and it
        # leaves every part of the pool machinery -- joint alpha, aligned
        # merging, joint distillation, checkpoint layout -- identical to the
        # continuous case. Only the divergence used to compare and distil
        # policies changes, and it changes for the better: an exact categorical
        # KL replaces a diagonal-Gaussian approximation to a tanh-squashed
        # policy.
    ...
    # ------------------------------------------------------------------
    # Policy divergence: the single place that knows the action-space type
    # ------------------------------------------------------------------
    def _symmetric_divergence(self, mean_a, log_a, mean_b, log_b):
        """Symmetric divergence between two policies, one scalar per state."""
        if self.discrete:
            return symmetric_categorical_kl(mean_a + log_a, mean_b + log_b)
        return symmetric_diagonal_gaussian_kl(mean_a, log_a, mean_b, log_b)

    def _forward_divergence(self, teacher_mean, teacher_log, student_mean, student_log):
        """KL(teacher || student), the distillation objective.

        In the continuous case callers pass ALREADY-BOUNDED log-stds, matching
        diagonal_gaussian_kl's contract. In the discrete case the two head
        outputs are summed into logits and the KL is exact, so no bounding is
        involved.
        """
        if self.discrete:
            return categorical_kl(teacher_mean + teacher_log, student_mean + student_log)
        return diagonal_gaussian_kl(teacher_mean, teacher_log, student_mean, student_log)
        ...
        with torch.no_grad():
            ...
            for i in range(n):
                for j in range(i + 1, n):
                    ...
                    skl = self._symmetric_divergence(mean_i, log_i, mean_j, log_j)
            ...
            selected_state_kl = self._symmetric_divergence(
    ...
    def _distill_pair(self, idx1: int, idx2: int):
        ...
        with torch.no_grad():
            ...
            teacher_logstd = teacher_raw_logstd if self.discrete else bound_log_std(teacher_raw_logstd)
        ...
        def kl_summary(indices):
            ...
            sl = sl_raw if self.discrete else bound_log_std(sl_raw)
            values = self._forward_divergence(
        ...
        for epoch in range(1, self.distill_epochs + 1):
            ...
            for start in range(0, shuffled.numel(), self.distill_batch_size):
                ...
                student_logstd = student_raw_logstd if self.discrete else bound_log_std(student_raw_logstd)
                loss = self._forward_divergence(
