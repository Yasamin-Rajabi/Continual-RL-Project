...
def categorical_kl(logits_p: torch.Tensor, logits_q: torch.Tensor) -> torch.Tensor:
    """KL(p || q) for categorical policies given unnormalized logits.

    Exact, unlike the diagonal-Gaussian KL used for continuous control, which
    approximates a tanh-squashed distribution by the pre-squash Gaussian. On a
    discrete action space the behavioural similarity between two pool entries
    is therefore measured without approximation error.
    """
