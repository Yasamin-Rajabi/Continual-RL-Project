"""Task-blind checkpoint evaluation for the MiniGrid paper suite.

``pool`` executes the FINALIZED categorical policy pool with one global set of
routing weights; no task identity is supplied. ``snapshot`` executes the exact
active categorical policy saved before insertion/consolidation. Optional
alpha-only test-time adaptation is explicitly counted and never creates or
updates a policy expert.
"""
...
def load_finalized_policy(run_dir, device):
    ...
    agent = CkaRlAgent(
        ...
        distill_observation_skip=bool(snapshot.get("distill_observation_skip", False)),
        encoder_linear_out=bool(snapshot.get("encoder_linear_out", False)),
        discrete=True,
    ).to(device)
    agent.pool_only = True
    agent.set_mixture_warmup(True)  # exact unit historical mass; no novel expert at test time
...
def evaluate(run_dir, suite, task_id, episodes, seed, device, *,
             ...
             action_mode="deterministic", error_key="task_error", episodic_success=True):
    ...
    if not hasattr(env.action_space, "n") or int(env.action_space.n) != policy.act_dim:
        env.close()
        raise ValueError("Checkpoint action dimension is incompatible with MiniGrid task")
        ...
        with torch.random.fork_rng(devices=cuda_devices):
            ...
            if adapt_steps:
                ...
                for _ in range(int(adapt_steps)):
                    ...
                    action, log_prob, _ = sample_action(policy, x, score_function=True)
                    obs, reward, terminated, truncated, _ = env.step(int(action[0].detach().item()))
                    adaptation_interactions += 1
                    if optimizer is not None:
                        # Same alpha-only immediate-reward routing heuristic as
                        # the continuous-control evaluator; only the categorical
                        # action distribution differs.
            ...
            for ep in range(episodes):
                ...
                while True:
                    x = torch.as_tensor(obs, device=device, dtype=torch.float32).unsqueeze(0)
                    with torch.no_grad():
                        action = (sample_action(policy, x)[0] if action_mode == "stochastic"
                                  else representative_action(policy, x))
                    obs, reward, terminated, truncated, info = env.step(int(action[0].item()))
