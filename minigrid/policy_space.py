...
It is therefore projected into a single categorical policy head pair before pool insertion,
...
from policy_composition import stacked_head_forward
...
class PolicySpaceMixin:
    ...
    def _components_at_features(self, z):
        ...
        head_a = stacked_head_forward(z, heads[0])
        head_b = stacked_head_forward(z, heads[1])
    ...
    def project_policy_for_storage(self):
        """Train one complete categorical policy to represent the current mixture.

        The componentwise forward-KL objective has the same minimizer as direct
        forward-KL projection of the executed categorical mixture.  The two
        stored heads remain separate for checkpoint compatibility, while their
        sum defines the student logits.
        """
        buffer = self.mean_pool.own_buffer
        ...
        with torch.no_grad():
            target_a, target_b, weights = self._components_at_features(z)
            target_a, target_b, weights = target_a.detach(), target_b.detach(), weights.detach()
        ...
        def values(params, indices):
            student_a = self.mean_pool._forward_with_weights(
                z[indices], tuple(params[0][k] for k in KEYS)
            )
            student_b = self.logstd_pool._forward_with_weights(
                z[indices], tuple(params[1][k] for k in KEYS)
            )
            teacher_logits = target_a[indices] + target_b[indices]
            student_logits = (student_a + student_b)[:, None, :]
            loss = categorical_kl(teacher_logits, student_logits)
