"""Bounded knowledge-pool storage for one squashed-Gaussian policy head.

One pool slot holds one *complete* policy head: the weights producing both the
mean and the log-std.  This is the continuous-control counterpart of the Atari
categorical logits pool, and the continual-learning mechanism is unchanged:

    theta_current = theta_own + sum_k alpha_k v_k

with the classic-CKA root added in ``classic_cka`` mode, or the reconstructed
full weights used directly in ``weight_delta`` mode.  Pool capacity,
alpha-weighted reuse, arithmetic merging, distillation replacement and replay
lineage all behave exactly as in the Atari implementation.
"""
...
def balanced_lineage_indices(source_ids, max_rows: int) -> np.ndarray:
    """Sample up to ``max_rows`` rows as evenly as possible across lineages.

    Without this, a slot that has absorbed several merges is dominated by
    whichever original task contributed most rows, and the behavioral KL then
    compares policies on a biased state distribution.
    """
    ...
    chosen = [
        np.random.choice(groups[sid], size=allocations[sid], replace=False)
        for sid in unique_ids
        if allocations[sid] > 0
    ]
    if not chosen:
        return np.empty((0,), dtype=np.int64)
...
class HeadPool(nn.Module):
    """Bounded storage and reuse of squashed-Gaussian policy-head parameters."""
    ...
    def __init__(
        ...
        if head_type != "gaussian":
            raise ValueError("PointMaze HeadPool requires head_type='gaussian'")
        ...
        self.head_type = "gaussian"
        self.shared_dim = int(shared_dim)
        self.hidden_dim = int(hidden_dim)
        self.act_dim = int(act_dim)
        # A head emits mean and log-std together.
        self.out_dim = 2 * int(act_dim)
        ...
        self.register_buffer("base_l0_weight", torch.zeros(self.hidden_dim, self.shared_dim))
        self.register_buffer("base_l0_bias", torch.zeros(self.hidden_dim))
        self.register_buffer("base_l2_weight", torch.zeros(self.out_dim, self.hidden_dim))
        self.register_buffer("base_l2_bias", torch.zeros(self.out_dim))

        self.own_l0_weight = Parameter(torch.empty(self.hidden_dim, self.shared_dim))
        self.own_l0_bias = Parameter(torch.empty(self.hidden_dim))
        self.own_l2_weight = Parameter(torch.empty(self.out_dim, self.hidden_dim))
        self.own_l2_bias = Parameter(torch.empty(self.out_dim))
    ...
    def _apply(self, fn, *args, **kwargs):
        # Historical pool entries are plain dicts of tensors, not Parameters or
        # registered buffers, so nn.Module._apply will not move them.
    ...
    def _historical(self):
        ...
        if self.alpha is None or self.alpha.numel() != len(self.pool):
            raise RuntimeError(
                "alpha length does not match pool length: "
    ...
    def _forward_with_weights(features, weights):
        w0, b0, w2, b2 = weights
        # Raw [mean, log_std] output; clamping and squashing belong to
        # policy_utils, not to the stored head.
    ...
    def inherit_pool_from(self, latest_pool: "HeadPool"):
        if getattr(latest_pool, "format_version", -1) != self.format_version:
            raise RuntimeError(
                "checkpoint uses an incompatible pool format; start a fresh chain"
            )
        if getattr(latest_pool, "head_type", None) != "gaussian":
            raise RuntimeError("checkpoint is not a Gaussian policy pool")
        ...
        for name in ("base_l0_weight", "base_l0_bias", "base_l2_weight", "base_l2_bias"):
            loaded = getattr(latest_pool, name, None)
            expected = tuple(getattr(self, name).shape)
            if loaded is None or tuple(loaded.shape) != expected:
                raise RuntimeError(
                    f"checkpoint {name} shape "
                    f"{None if loaded is None else tuple(loaded.shape)} "
        ...
        for entry in latest_pool.pool:
            ...
            # Replay buffers are read-only NumPy arrays here, so sharing the
            # object rather than copying avoids duplicating stored states.
    ...
    def set_base(self):
        ...
        with torch.no_grad():
            ...
            if self.fusion_mode == BASE_FUSION_MODE:
                entry = {k: torch.zeros_like(getattr(self, "own_" + k).data) for k in _HEAD_KEYS}
            else:
                entry = {k: getattr(self, "own_" + k).data.clone() for k in _HEAD_KEYS}
    ...
    def finalize_own_contribution(self):
        if self.fusion_mode == "weight_delta":
            hist = self._historical()
            entry = {
                k: (getattr(self, "own_" + k).data + hist[k]).clone() for k in _HEAD_KEYS
            }
        else:
            entry = {k: getattr(self, "own_" + k).data.clone() for k in _HEAD_KEYS}
        entry["buffer"] = self.own_buffer
    ...
    def average_pair_params(self, idx1: int, idx2: int) -> Dict[str, torch.Tensor]:
        return {
            key: 0.5 * (self.pool[int(idx1)][key] + self.pool[int(idx2)][key])
            for key in _HEAD_KEYS
        }
    ...
    def replace_pair(self, idx1, idx2, params, merged_buffer, merge_info):
        ...
        if idx1 == idx2:
            raise ValueError("merge pair must contain two distinct entries")
        n = len(self.pool)
        entry = {key: params[key].detach().clone() for key in _HEAD_KEYS}
        entry["buffer"] = merged_buffer
        # Surviving entries keep their order; the merged item is appended.
    ...
    def _trim_buffer(buffer, max_rows):
        ...
        if n <= max_rows:
            return dict(buffer)
        idx = np.random.choice(n, size=int(max_rows), replace=False)
        return {
    ...
    @staticmethod
    def _gather_two(buf1, buf2, key, idx1, idx2):
        a, b = buf1[key], buf2[key]
        out = np.empty((len(idx1) + len(idx2),) + a.shape[1:], dtype=a.dtype)
        np.take(a, idx1, axis=0, out=out[: len(idx1)])
        np.take(b, idx2, axis=0, out=out[len(idx1):])
        return out

    @staticmethod
    def merge_buffers(buf1, buf2, max_rows: int, balance_source_lineages: bool = False):
        """Merge two lineage buffers under a hard row budget.

        Every row-aligned array receives the same sampled indices, so
        observation / action / task / source correspondence is preserved.
        """
        ...
        keys = [
            k for k in buf1
            if k in buf2
            and isinstance(buf1[k], np.ndarray)
            and isinstance(buf2[k], np.ndarray)
        ]
        if "obs" not in keys:
        ...
        if n1 + n2 <= max_rows:
            return {k: np.concatenate([buf1[k], buf2[k]], axis=0) for k in keys}

        if balance_source_lineages:
            if "source_ids" not in keys:
                raise RuntimeError("balanced merging requires source_ids in both buffers")
            combined_sources = np.concatenate(
                [buf1["source_ids"], buf2["source_ids"]], axis=0
            )
            idx = balanced_lineage_indices(combined_sources, max_rows)
            idx1 = idx[idx < n1]
            idx2 = idx[idx >= n1] - n1
            return {k: HeadPool._gather_two(buf1, buf2, k, idx1, idx2) for k in keys}
